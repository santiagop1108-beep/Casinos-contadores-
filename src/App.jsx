...
"simulacion":{"m":[{"id":"M1","nombre":"Maq 1","factor":1},{"id":"M2","nombre":"Multi 2","factor":10},{"id":"P3","nombre":"Poker 3","factor":50},{"id":"JW4","nombre":"Jungle Wild 4","factor":1},{"id":"M5","nombre":"Multi 5","factor":10},{"id":"M6","nombre":"Multi 6","factor":10},{"id":"M7","nombre":"Multi 7","factor":10},{"id":"M8","nombre":"Multi 8","factor":10},{"id":"M9","nombre":"Multi 9","factor":10},{"id":"M10","nombre":"Multi 10","factor":10},{"id":"M11","nombre":"Multi 11","factor":10},{"id":"M12","nombre":"Multi 12","factor":10},{"id":"M13","nombre":"Multi 13","factor":10},{"id":"M14","nombre":"Multi 14","factor":10},{"id":"M15","nombre":"Multi 15","factor":10},{"id":"M16","nombre":"Multi 16","factor":10},{"id":"M17","nombre":"Multi 17","factor":10},{"id":"D18","nombre":"Duende 18","factor":1},{"id":"M19","nombre":"Multi 19","factor":10},{"id":"M20","nombre":"Multi 20","factor":10}],"ul":{},"b":[]}
};
...
const COL_MAP={
  ...
  "simulacion":{"Multi 16":{"p":5,"u":6},"Multi 13":{"p":5,"u":6},"Jungle Wild 4":{"p":4,"u":5},"Multi 15":{"p":5,"u":6},"Multi 2":{"p":5,"u":6},"Multi 8":{"p":5,"u":6},"Multi 12":{"p":5,"u":6},"Maq 1":{"p":4,"u":5},"Multi 7":{"p":5,"u":6},"Multi 11":{"p":5,"u":6},"Multi 6":{"p":5,"u":6},"Multi 9":{"p":5,"u":6},"Multi 20":{"p":5,"u":6},"Duende 18":{"p":4,"u":5},"Multi 14":{"p":5,"u":6},"Multi 10":{"p":5,"u":6},"Poker 3":{"p":6,"u":7},"Multi 19":{"p":5,"u":6},"Multi 5":{"p":5,"u":6},"Multi 17":{"p":5,"u":6}},
};
...
// ─── GOOGLE DRIVE ─────────────────────────────────────────────────────────────
async function compressImage(file,maxW=1400,quality=0.85){return new Promise(resolve=>{const img=new Image();img.onload=()=>{const scale=Math.min(1,maxW/img.width);const w=Math.round(img.width*scale),h=Math.round(img.height*scale);const c=document.createElement("canvas");c.width=w;c.height=h;c.getContext("2d").drawImage(img,0,0,w,h);c.toBlob(b=>resolve(b),"image/jpeg",quality);};img.src=URL.createObjectURL(file);});}
...
function Camera({cid,cont,setCont,apiKey}){
  ...
  async function analyzePhoto(blob,idx){
    ...
    try{
      const mq_list=mqs.map(q=>{const tipo=q.factor===50?"POKER":q.factor===1?"ESPECIAL":"MULTI";const stickerNum=q.nombre.match(/\d+$/)?.[0]||"";return"Num:"+stickerNum+" | ID:"+q.id+" | "+q.nombre+" | "+tipo;}).join("\n");
      const prompt="Eres experto en contadores de tragamonedas colombianas.\n\nMÁQUINAS DISPONIBLES:\n"+mq_list+"\n\nPASO 1 — IDENTIFICA LA MÁQUINA:\nBusca el número físico pegado en el MARCO o BORDE de la máquina (no en la pantalla). Puede estar en cualquier esquina o lateral. Si no lo ves claramente, pon maq_numero: null y maq_id_asignado: null, pero IGUAL extrae los contadores de la pantalla.\nEl número del sticker coincide con el Num de la lista (ej: sticker '8' → Num 8 → ID M8, sticker '13' → ID M13, sticker '4' → ID JW4, sticker '18' → ID D18, sticker '3' → ID P3, sticker '1' → ID M1).\n\nPASO 2 — EXTRAE CONTADORES según el tipo de pantalla:\n\nMULTI / GAMINATOR (pantalla azul con 'STATISTICS'):\n• Si hay 2 columnas: lee SOLO columna IZQUIERDA (SHORT STATISTICS), ignora columna derecha\n• total_in=TOTAL IN, total_out=TOTAL OUT, jackpot=null\n\nPOKER IGT (pantalla azul, lista larga de campos contables):\n• total_in = campo DROP exactamente (acumulador de billetes físicos — es MENOR que COINS IN)\n• total_out = campo PHYSICAL COINS OUT exactamente (NO es YIELD, NO es COINS OUT)\n• jackpot = campo JACKPOTS\n• IGNORAR: COINS IN, PHYSICAL COINS IN, YIELD, TOP WINS, TOTAL WINS, LOSSES\n\nDUENDE 18 / D18 — sticker 18 (pantalla azul dividida IZQUIERDA/DERECHA):\n• La pantalla tiene 2 mitades lado a lado\n• Lee SOLO la mitad con valores MENORES (SHORT STATISTICS, usualmente izquierda)\n• IGNORA la mitad con valores ENORMES (LONG STATISTICS, > 100,000,000)\n• total_in=COINS IN o TOTAL IN de la mitad pequeña, total_out=COINS OUT o TOTAL OUT de la mitad pequeña\n\nJUNGLE WILD / JW4 — sticker 4 (pantalla VERDE 'Contadores'):\n• Usa 'Contadores Parciales' si existe, si no 'Contadores Totales'\n• total_in=Acreditado, total_out=Pagado\n\nMAQ 1 / M1 — sticker 1 (pantalla VERDE tabla):\n• Columna 'Principios' o 'Parciales' (NO Totales)\n• total_in=Acreditado/Introducido, total_out=Pagado/Devuelto\n\nJSON sin markdown:\n{\"maq_numero\":<numero o null>,\"maq_id_asignado\":\"<ID o null>\",\"tipo\":\"<MULTI|POKER|ESPECIAL>\",\"total_in\":<entero>,\"total_out\":<entero>,\"jackpot\":<entero o null>,\"confianza\":\"<alta|media|baja>\",\"notas\":\"<max 15 palabras>\"}";
      ...
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",
        ...
        body:JSON.stringify({model:"claude-sonnet-4-6",max_tokens:700,system:"Responde ÚNICAMENTE con un objeto JSON válido. Nunca escribas texto antes ni después del JSON.",messages:[{role:"user",content:[{type:"image",source:{type:"base64",media_type:"image/jpeg",data:b64}},{type:"text",text:prompt}]}]})});
      ...
      // Notificar éxito OCR a Jarvis
      try{fetch("https://ntfy.sh/jarvis",{method:"POST",headers:{"Title":"✅ OCR OK — Casino Contadores","Priority":"low","Tags":"white_check_mark,casino"},body:`OCR exitoso en ${m.n}: máquina ${parsed.maq_numero||assignedId||'desconocida'} (${parsed.tipo||'tipo desconocido'}) · confianza ${parsed.confianza||'media'}`});}catch{}
    }catch(e){setQueue(q=>q.map((x,i)=>i===idx?{...x,status:"error",err:e.message}:x));
      // Notificar error OCR a Jarvis
  ...
  async function reanalyzeWithMachine(idx){
    ...
    else if(tipo==="POKER")regla="Pantalla azul POKER IGT con lista de campos contables.\ntotal_in = campo DROP exactamente (acumulador de billetes, es MENOR que COINS IN)\ntotal_out = campo PHYSICAL COINS OUT exactamente (NO es YIELD ni COINS OUT)\njackpot = campo JACKPOTS\nIgnorar: COINS IN, PHYSICAL COINS IN, YIELD, TOP WINS, TOTAL WINS";
    ...
    try{
      ...
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",
        headers:{"Content-Type":"application/json","x-api-key":apiKey,"anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-access":"true"},
        body:JSON.stringify({model:"claude-sonnet-4-6",max_tokens:400,system:"Responde ÚNICAMENTE con un objeto JSON válido. Nunca escribas texto antes ni después del JSON.",messages:[{role:"user",content:[{type:"image",source:{type:"base64",media_type:"image/jpeg",data:b64}},{type:"text",text:prompt}]}]})});
      const data=await res.json();
      const rawText=data.content?.[0]?.text||"{}";
      const raw=rawText.replace(/```json|```/g,"").trim();
      const jsonMatch=raw.match(/\{[\s\S]*\}/);
...
function Report({cid,cont}){
  ...
  async function handleGenerarEnviar(){
    ...
    try{
      ...
      setEmailStatus("sent");
      try{fetch("https://jarvis-ntfy-production.up.railway.app/jarvis",{method:"POST",headers:{"Title":"📊 Reporte enviado","Priority":"default","Tags":"casino,reporte"},body:`Reporte enviado a ${rEmail.trim()} — ${m.n} ${label}`});}catch{}
