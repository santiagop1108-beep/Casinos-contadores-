...
function Camera({cid,cont,setCont,apiKey}){
  ...
  async function analyzePhoto(blob,idx){
    ...
    try{
      const mq_list=mqs.map(q=>{const tipo=q.factor===50?"POKER":q.factor===1?"ESPECIAL":"MULTI";const stickerNum=q.nombre.match(/\d+$/)?.[0]||"";return"- Sticker:"+stickerNum+" → ID:"+q.id+" | "+q.nombre+" | "+tipo;}).join("\n");
      const prompt="Eres experto en contadores de tragamonedas colombianas.\n\nMÁQUINAS (número sticker físico → ID de la app):\n"+mq_list+"\n\nPASO 1 — IDENTIFICA LA MÁQUINA:\nBusca un número pegado en el MARCO FÍSICO o esquina de la foto (NO leas números de la pantalla). Ese número es el Sticker. Encuentra en la lista el ID cuyo Sticker coincide con el número visto.\n\nPASO 2 — EXTRAE CONTADORES según el tipo de pantalla:\n\nMULTI / GAMINATOR (pantalla azul con 'STATISTICS'):\n• Si hay 2 columnas (SHORT/LONG): lee SOLO la columna IZQUIERDA (SHORT STATISTICS)\n• total_in=TOTAL IN, total_out=TOTAL OUT, jackpot=null\n\nPOKER IGT (pantalla azul, lista de campos con DROP):\n• total_in=DROP (dinero en billetes — NO es COINS IN ni TOTAL IN)\n• total_out=PHYSICAL COINS OUT, jackpot=JACKPOTS\n\nDUENDE 18 / D18 (pantalla azul, 2 SECCIONES VERTICALES arriba/abajo):\n• IGNORA completamente la sección SUPERIOR (valores enormes > 100,000,000)\n• Lee SOLO la sección INFERIOR (SHORT STATISTICS)\n• total_in=COINS IN sección inferior, total_out=COINS OUT o PHYSICAL COINS OUT sección inferior\n\nJUNGLE WILD / JW4 (pantalla VERDE con título 'Contadores'):\n• Si existe 'Contadores Parciales': usa esa sección; si no, usa 'Contadores Totales'\n• total_in=Acreditado (primer valor), total_out=Pagado (segundo valor)\n\nMAQ 1 / M1 (pantalla VERDE, tabla con filas y columnas):\n• Usa columna 'Principios' o 'Parciales' (NO columna Totales)\n• total_in=fila Acreditado o Introducido, total_out=fila Pagado o Devuelto\n\nResponde SOLO con este JSON (sin markdown, sin texto extra):\n{\"maq_numero\":<sticker visto>,\"maq_id_asignado\":\"<ID de la lista>\",\"tipo\":\"<MULTI|POKER|ESPECIAL>\",\"total_in\":<entero sin puntos>,\"total_out\":<entero sin puntos>,\"jackpot\":<entero o null>,\"confianza\":\"<alta|media|baja>\",\"notas\":\"<max 20 palabras>\"}";
