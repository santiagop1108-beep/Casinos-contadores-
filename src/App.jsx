...
function Camera({cid,cont,setCont,apiKey}){
  ...
  async function analyzePhoto(blob,idx){
    ...
    try{
      ...
      const prompt="Eres experto en contadores de tragamonedas colombianas.\nMÁQUINAS:\n"+mq_list+"\n\nIDENTIFICA el número de máquina (esquina sup-izq de la FOTO, no de la pantalla) y EXTRAE contadores exactos.\n\nREGLAS POR TIPO DE PANTALLA:\n\nMULTI (pantalla azul SHORT STATISTICS):\n  total_in=TOTAL IN, total_out=TOTAL OUT, jackpot=null\n\nADMIRAL (pantalla azul dividida en 2 columnas):\n  Lee SOLO la columna IZQUIERDA (SHORT STATISTICS). IGNORA columna derecha (LONG STATISTICS).\n  total_in=TOTAL IN columna izq, total_out=TOTAL OUT columna izq\n\nPOKER IGT (pantalla azul lista de campos):\n  total_in=DROP (dinero en billetes, NO es COINS IN ni TOP WINS)\n  total_out=PHYSICAL COINS OUT, jackpot=JACKPOTS\n\nDuende 18 / D18 (pantalla azul monocromática 2 secciones verticales):\n  IGNORA sección SUPERIOR (LONG STATISTICS / CENTS IN - valores enormes)\n  Lee sección INFERIOR (SHORT STATISTICS): total_in=COINS IN, total_out=COINS OUT o PHYSICAL COINS OUT\n\nJungle Wild / JW4 (pantalla verde 'Contadores'):\n  Busca sección 'Contadores Parciales' si existe, si no usa 'Contadores Totales'\n  total_in=Acreditado o primer valor, total_out=Pagado o segundo valor\n\nGame Chest / Maq 1 (pantalla verde tabla):\n  Busca columna 'Principios' o 'Parciales' en la tabla\n  total_in=fila Acreditado/Introducido, total_out=fila Pagado/Devuelto\n\nJSON sin markdown: {maq_numero,maq_id_asignado,tipo,total_in,total_out,jackpot,confianza,notas}";
      ...
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",
        ...
        body:JSON.stringify({model:"claude-sonnet-4-6",max_tokens:512,messages:[{role:"user",content:[{type:"image",source:{type:"base64",media_type:"image/jpeg",data:b64}},{type:"text",text:prompt}]}]})});
